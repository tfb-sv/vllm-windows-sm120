...
__global__ void silu_mul_fp8_quant_deep_gemm_kernel(
  ...
  // We pack 8 16-bit bfloat16 values into a 128-bit int128_t.
  ...
  __shared__ alignas(16) int128_t s_buff_128[S_NUM_128];
  ...
  auto input_128_ptr = reinterpret_cast<const int128_t*>(_input);
  ...
  int64_t* s_gate_ptr = reinterpret_cast<int64_t*>(
                              s_buff_compute_32 + warp_id * (GROUP_SIZE / 2)) +
                          lane_id;
  int64_t* s_up_ptr = s_gate_ptr + S_NUM_64 / 2;
